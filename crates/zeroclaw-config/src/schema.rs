...
pub struct Config {
    ...
    /// WaveSpeed TTS tool configuration (`[wavespeed_tts]`).
    #[serde(default)]
    #[nested]
    pub wavespeed_tts: WaveSpeedTtsConfig,
...
/// WaveSpeed TTS tool configuration (`[wavespeed_tts]`).
///
/// Companion to the existing `image_gen` tool: same vendor, same
/// `WAVESPEED_API_KEY` convention, distinct allowlist of TTS models.
#[derive(Debug, Clone, Serialize, Deserialize, Configurable)]
#[cfg_attr(feature = "schema-export", derive(schemars::JsonSchema))]
#[prefix = "wavespeed-tts"]
pub struct WaveSpeedTtsConfig {
    /// Enable the `wavespeed_tts` tool.
    #[serde(default)]
    pub enabled: bool,
    /// WaveSpeed API key. Falls back to the env var named in `api_key_env`.
    #[secret]
    #[cfg_attr(feature = "schema-export", schemars(extend("x-secret" = true)))]
    #[serde(default)]
    pub api_key: Option<String>,
    /// Env var to read when `api_key` is unset. Default: `WAVESPEED_API_KEY`
    /// (matches `image_gen` so operators don't manage two keys).
    #[serde(default = "default_wavespeed_tts_api_key_env")]
    pub api_key_env: String,
    /// Allowed WaveSpeed model paths. Use `["*"]` to allow any model.
    #[serde(default)]
    pub allowed_models: Vec<String>,
    /// Default voice when the agent doesn't pass one. Must be valid for
    /// the chosen model — defaults are model-specific.
    #[serde(default = "default_wavespeed_tts_voice")]
    pub default_voice: String,
    /// Default language code or "auto".
    #[serde(default = "default_wavespeed_tts_language")]
    pub default_language: String,
    /// Output format for the generated audio file: `mp3` (default), `wav`,
    /// `opus`, `flac`. Used only for the autogenerated filename — the
    /// actual format is whatever WaveSpeed returns.
    #[serde(default = "default_wavespeed_tts_output_format")]
    pub output_format: String,
    /// Hard cap on input character length (TTS bills per character).
    #[serde(default = "default_wavespeed_tts_max_input_chars")]
    pub max_input_chars: usize,
    /// Per-HTTP-request timeout in seconds.
    #[serde(default = "default_wavespeed_tts_request_timeout_secs")]
    pub request_timeout_secs: u64,
    /// End-to-end timeout (submit + poll + download).
    #[serde(default = "default_wavespeed_tts_total_timeout_secs")]
    pub total_timeout_secs: u64,
    /// Max audio download size (50 MiB default — long VibeVoice podcasts
    /// can be tens of MB).
    #[serde(default = "default_wavespeed_tts_max_download_bytes")]
    pub max_download_bytes: u64,
}

fn default_wavespeed_tts_api_key_env() -> String {
    "WAVESPEED_API_KEY".into()
}
fn default_wavespeed_tts_voice() -> String {
    "Vivian".into()
}
fn default_wavespeed_tts_language() -> String {
    "auto".into()
}
fn default_wavespeed_tts_output_format() -> String {
    "mp3".into()
}
fn default_wavespeed_tts_max_input_chars() -> usize {
    5_000
}
fn default_wavespeed_tts_request_timeout_secs() -> u64 {
    60
}
fn default_wavespeed_tts_total_timeout_secs() -> u64 {
    300
}
fn default_wavespeed_tts_max_download_bytes() -> u64 {
    52_428_800
}

impl Default for WaveSpeedTtsConfig {
    fn default() -> Self {
        Self {
            enabled: false,
            api_key: None,
            api_key_env: default_wavespeed_tts_api_key_env(),
            allowed_models: Vec::new(),
            default_voice: default_wavespeed_tts_voice(),
            default_language: default_wavespeed_tts_language(),
            output_format: default_wavespeed_tts_output_format(),
            max_input_chars: default_wavespeed_tts_max_input_chars(),
            request_timeout_secs: default_wavespeed_tts_request_timeout_secs(),
            total_timeout_secs: default_wavespeed_tts_total_timeout_secs(),
            max_download_bytes: default_wavespeed_tts_max_download_bytes(),
        }
    }
}
...
impl Default for Config {
    fn default() -> Self {
        ...
        Self {
            ...
            wavespeed_tts: WaveSpeedTtsConfig::default(),
